...
@permission_required('sims102.index102_contributor')
def IndexListView(request):
    object_list = Index102.objects.all() 
    first = Index102.objects.first()  
    # description= Description.objects.get(sequence=Index102.SEQUENCE)
    description = get_object_or_404(Description, sequence=Index102.SEQUENCE)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():              
            ...
            return render(request, 'sims102/index_list.html', context)
    else:
        form = IndexForm()
        context = {'form':form, 'object_list':object_list, 'first':first, 'description':description}
    return render(request, 'sims102/index_list.html', context)
...
def ajax_change_session(request):  
    ...
    return render(request, 'sims102/index_delete.html') 
...
def ajax_calculate(request):     ###  must be the same as 'calculate' function  in model.py 
    ...
    calculated_value = ((data_one) / data_two) * 100000
    calculated_value = format(calculated_value, '.2f')
    return render(request, 'sims102/calculated_value.html', {'calculated_value':calculated_value})

class IndexUpdateView(PermissionRequiredMixin, UpdateView):
    permission_required = ('sims102.index102_validator') 
    model = Index102
    form_class = IndexForm
    template_name = 'sims102/index_update.html'
    success_url = reverse_lazy('sims102:index_list')  

# @permission_required('sims102.index102_validator')
# def IndexUpdateView(request, pk): 

#     obj = get_object_or_404(Index102, pk=pk)
...
#         return render(request, 'sims102/index_update.html', context)
...
class IndexDeleteView(PermissionRequiredMixin, DeleteView):
    permission_required = ('sims102.index102_validator')    
    model = Index102
    template_name = 'sims102/index_delete.html'
    success_url = reverse_lazy('sims102:index_list')  
...
def export_xls(request):
    ...
    font_style = xlwt.XFStyle()
    rows = Index102.objects.all().values_list('data_one', 'data_two', 'calculated_value')
    for row in rows:
        row_num += 1
...
#     permission_required = ('sims102.index-contributor') 
#     model = Index102
#     form_class = IndexForm 
...
#         return render(self.request, 'sims102/index_list.html', context)

#         # return HttpResponseRedirect('/102/')
...
#         context['first'] = Index102.objects.first()  
#         context['description'] = Description.objects.get(sequence=Index102.SEQUENCE)
...
#     permission_required = ('sims102.index-contributor') 
#     model = Index102
#     template_name = 'sims102/index_detail.html' 